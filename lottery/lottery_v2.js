...
function Lottery(option) {
    ...
    var _private = {
        ...
        isMobile: function () {
            return /iphone|ios|android|mini|mobile|mobi|Nokia|Symbian|iPod|iPad|Windows\s+Phone|MQQBrowser|wp7|wp8|UCBrowser7|UCWEB|360\s+Aphone\s+Browser|blackberry/i
        ...
        findKeyframesRule: function (rule) {
            ...
            for (var i = 0; i < ss.length; ++i) {
                ...
                for (var j = 0; j < ss[i].cssRules.length; ++j) {
                    if (ss[i].cssRules[j].type == (window.CSSRule.WEBKIT_KEYFRAMES_RULE || window.CSSRule.MOZ_KEYFRAMES_RULE ||
                        window.CSSRule.O_KEYFRAMES_RULE || window.CSSRule.KEYFRAMES_RULE) && ss[i].cssRules[j].name ==
                        rule)
        ...
        change: function (anim, style, from, to) {
            ...
            if (keyframes) {
                ...
                var insert = function (rule) {
                    return (keyframes.appendRule && keyframes.appendRule(rule)) || (keyframes.insertRule && keyframes.insertRule(
                        rule));
                }
                insert("from { " + style + ": rotateZ(" + from + "deg); }");
                insert("to { " + style + ": rotateZ(" + to + "deg); }");
            } else {
                _private.addRule('@' + _private.prefix + 'keyframes ' + anim, 'from { ' + style + ': rotateZ(' + from +
                    'deg); } to { ' + style + ': rotateZ(' + to + 'deg); }');
            }
        }
    }
    ...
    if (_private.isMobile() && config.isResponsive) {
        //alert(1);
    ...
    var startBtn = null,
        moveBox = null,
        container = document.getElementById(config['contentId']);
    ...
    var classInit = (function () {
        ...
        if (config.r) { //大转盘类
            ...
            //不可点击状态的抽奖按钮class初始化
            var disableBtn = extendObj(startBtn, {
                cursor: 'normal'
            });
            ...
            if (myBrowser != "IE6" && myBrowser != "IE7" && myBrowser != "IE8" && myBrowser != "IE9" && myBrowser !=
                "edge") {
                _private.addRule('@' + _private.prefix + 'keyframes ' + classes['bgLight'],
                    '0% {background-position: -500px 0}100% {background-position: 500px 0}');
                _private.addRule('@' + _private.prefix + 'keyframes ' + classes['borderLight'],
                    '0% {box-shadow:0px 0px 10px 10px rgba(255,255,255,.3) inset}100% {box-shadow:0px 0px 20px 20px rgba(255,255,255,.6) inset}');
            }

            //抽奖按钮的class初始化
            var startBtn = {
                position: 'absolute',
                width: config.sbtnw + config.unit,
                height: config.sbtnh + config.unit,
                left: config.sbtnx + config.unit,
                top: config.sbtny + config.unit,
                display: 'block',
                outline: 'none'
            }
            //正常状态下的抽奖按钮
            var start = extendObj(startBtn, {
                cursor: 'pointer',
            ...
            config.lighturl ? (moveLight['background'] = "url(" + config.lighturl + ") no-repeat") : (moveLight[
                'box-shadow'] = '0px 0px 10px 10px rgba(255,255,255,.8) inset');
            moveLight['background-size'] = "cover";
            //移动的光影效果
    ...
    var faceInit = (function () {
        ...
        if (config.r) {
            str = '<div  class="' + classes['container'] +
                '"><div id="' + classes['hover'] + '" class="' + classes['hover'] + '"></div><img src="' + config.s +
                '" id="' + classes['start'] + '" class="' + classes['start'] + '"></div>';
        } else {
            str = '<div class="' + classes['container'] +
        ...
        if (config.r) { //调整坐标位置
            startBtn.onload = function () {
                startBtn.style.cssText = 'margin-left:' + -(startBtn.width / 2 + config.sx) + 'px; margin-top:' + -(
                    startBtn.height / 2 + config.sy) + config.unit;
    ...
    var moveNext = function () {
        ...
        moveBox.style.cssText = "display:block;left:" + curInfo.left + config.unit + ';' + "top:" + curInfo.top +config.unit+";transform:"+curInfo.transform;
    }
    ...
    this.stopRoll = function (id) {
        if (config.r) {
            if (myBrowser != "IE6" && myBrowser != "IE7" && myBrowser != "IE8" && myBrowser != "IE8" && myBrowser !=
                "edge") {
                var animateEndBool = false;
                moveBox.style.cssText = "-ms-transform: rotate(" + ((360 - step[id]) + 360 * 10) +
                    "deg); transform: rotateZ(" + ((360 - step[id]) + 360 * 10) +
                    "deg); transition:transform 5s cubic-bezier(0.35, -0.005, 0.565, 1.005) 0s;-ms-transition:transform 5s cubic-bezier(0.35, -0.005, 0.565, 1.005) 0s;";
                // 动画结束时事件
                moveBox.addEventListener("transitionend", function () {
                    if (!animateEndBool) {
                        config.onCompleteRollEvent();
                        moveBox.style.cssText = "-ms-transform: rotateZ(" + ((360 - step[id])) +
                            "deg); transform: rotateZ(" + ((360 - step[id])) + "deg);";
                        btn.enable();
                        animateEndBool = true;
                    }
                });
